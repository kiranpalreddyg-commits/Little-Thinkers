...
const QWEN_TIMEOUT_MS = 5_000
const QWEN_RETRY_COUNT = 2
...
async function callQwen(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`QWEN ${res.status}: ${body.slice(0, 200)}`)
    }
...
async function callWithRetry(
  ...
  for (let attempt = 0; attempt <= QWEN_RETRY_COUNT; attempt++) {
    try {
      return await callQwen(prompt, timeoutMs)
    } catch (err) {
      lastError = err instanceof Error ? err : new Error(String(err))
      if (lastError.name === 'AbortError') break  // hard timeout — don't retry
      if (attempt < QWEN_RETRY_COUNT) await sleep(200 * 2 ** attempt)
    }
  }
  throw lastError
}
...
export async function generateBridgeGame(
  ...
  const t0 = Date.now()
  // Bridge games are simpler — 3-question micro-game in one payload
